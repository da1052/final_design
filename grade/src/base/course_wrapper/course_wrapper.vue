  ...
  <div>
    <div class="course_wrapper" v-if="detail_flag == false">
      ...
      <div class="content">
        ...
        <div class="course_title">{{data_course.name}}教程</div>
        <div class="course_btn">
          <div
            class="btn_s"
            @click="show_detail(data_course_detail.detail[0].detail[0].catanum)"
          >开始阅读</div>
        ...
        <div class="course_summary">{{data_course.summary}}</div>
      </div>
    </div>
    <yd-popup v-model="popup_flag" position="right" width="60%">
      <div class="loading" v-show="!data_course_detail.detail">
        <!-- -->
      ...
      <scroll :data="data_course_detail.detail" :pulldown="pulldown" class="detail_wrapper">
        <ul>
          <li v-for="item in data_course_detail.detail" :key="item.title" class="detail_p">
            <h3>{{item.title}}</h3>
            <ul>
              <li
                v-for="item_s in item.detail"
                class="detail_s"
                :key="item_s.catalog"
                @click="show_detail(item_s.catanum)"
              >
                <!---->
    ...
    <div class="course_detail" v-if="detail_flag && this.detail != null">
      ...
      <div class="footer">
        <ul>
          <li
            :class="{check:detail[0].forward != ''}"
            @click="show_detail(detail[0].forward.catanum)"
          >
            <svg class="icon" aria-hidden="true">
...
export default {
  ...
  methods: {
    ...
    show_detail(num) {
      ...
      if (num == undefined) {
        ...
        this.$http.get("/detail?catanum=" + num).then(
          response => {
  ...
  created() {
    this.$http.get("/classify/" + this.data_course.name).then(
      response => {
