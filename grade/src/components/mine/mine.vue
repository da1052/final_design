        ...
        <div class="m_photo">
          <div class="photo" v-if="login_flag">
            ...
            <fileupload
              ...
              style="width: 5rem; height: 5rem;position: relative;top: -6.3rem;opacity: 0;"
            ></fileupload>
          </div>
          <router-link
            to="/Login"
            class="login"
            v-if="login_flag == false || login_flag == 'false'"
            tag="div"
          >登录/注册</router-link>
        </div>
...
export default {
  ...
  methods: {
    ...
    finishUpload(e) {
      ...
      let reg = /^(\s|\S)+(jpg|png|jpeg|heic|JPG|PNG|JPEG|HEIC)+$/;
      let flag = reg.test(resobj.file[0].originalFilename);
      console.log(resobj);
      if (flag == true) {
        this.$http
          .post("/user_img_update", {
  ...
  computed: {
    ...mapState(["login_flag", "header_text", "user_img", "user"])
  },
  components: {
